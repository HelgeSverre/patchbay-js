...
class d {
  ...
  update(t) {
    const e = this.x - this.oldX, s = this.y - this.oldY + t;
...
class p {
  ...
  constructor(t = {}) {
    ...
    this.config = { ...this.config, ...t }, this.start = this.config.start, this.end = this.config.end, this.isDragging = !1, this.draggedEnd = null, this.element = this.createMainElement(), this.points = this.initializePoints(), this.segmentElements = this.createSegmentElements(), this.config.draggable && (this.setupDragHandles(), this.setupDragEvents());
  }
  createMainElement() {
    const t = document.createElement("div");
    return t.classList.add("cable-container"), t.style.position = "fixed", t.style.top = "0", t.style.left = "0", t.style.pointerEvents = "none", t;
  }
  initializePoints() {
    const t = this.getElementCenter(this.start), e = this.getElementCenter(this.end), s = [new d(t.x, t.y)], n = e.x - t.x, i = e.y - t.y, o = Math.sqrt(n * n + i * i) * this.config.slack / (this.config.segments - 1);
    for (let r = 1; r < this.config.segments - 1; r++) {
      const h = r / (this.config.segments - 1);
      s.push(
        new d(
          t.x + n * h + (Math.random() - 0.5) * o,
          t.y + i * h + (Math.random() - 0.5) * o
        )
      );
    }
  ...
  setupDragHandles() {
    this.startHandle = this.createDragHandle(), this.endHandle = this.createDragHandle(), this.element.appendChild(this.startHandle), this.element.appendChild(this.endHandle);
  }
  setupDragEvents() {
    this.startHandle.addEventListener("mousedown", (t) => this.startDragging(t, "start")), this.endHandle.addEventListener("mousedown", (t) => this.startDragging(t, "end")), document.addEventListener("mousemove", (t) => this.drag(t)), document.addEventListener("mouseup", (t) => this.stopDragging(t));
  }
  createDragHandle() {
    const t = document.createElement("div");
    return t.classList.add("cable-drag-handle"), t.style.width = `${this.config.dragHandleSize}px`, t.style.height = `${this.config.dragHandleSize}px`, t.style.position = "absolute", t.style.borderRadius = "50%", t.style.backgroundColor = "transparent", t.style.pointerEvents = "all", t.style.cursor = "move", t.style.zIndex = "9999", t;
  }
  startDragging(t, e) {
  ...
  drag(t) {
    ...
    this.draggedEnd === "start" ? (this.points[0].x = e, this.points[0].y = s) : (this.points[this.points.length - 1].x = e, this.points[this.points.length - 1].y = s), this.update();
  }
  stopDragging() {
    if (!this.isDragging)
      return;
    const t = this.draggedEnd === "start" ? this.points[0] : this.points[this.points.length - 1], e = this.findSnapTarget(t.x, t.y);
    if (e) {
  ...
  findSnapTarget(t, e) {
    ...
    let n = null, i = 1 / 0;
    for (let a of s) {
      const o = a.getBoundingClientRect(), r = o.left + o.width / 2, h = o.top + o.height / 2, c = Math.sqrt((t - r) ** 2 + (e - h) ** 2);
      c < i && (i = c, n = a);
    }
  ...
  createSegmentElements() {
    return this.points.slice(0, -1).map((t, e) => {
      ...
      return s.classList.add("cable-segment"), s.style.position = "absolute", s.style.background = this.config.color, s.style.height = this.config.lineThickness + "px", s.style.transformOrigin = "0 50%", this.element.appendChild(s), s;
    });
  }
  update() {
    if (this.start) {
    ...
    for (let t = 1; t < this.points.length - 1; t++)
      this.points[t].y += this.config.gravity;
  ...
  solveConstraints() {
    ...
    for (let e = 0; e < this.points.length - 1; e++) {
      const s = this.points[e], n = this.points[e + 1], i = n.x - s.x, a = n.y - s.y, o = Math.sqrt(i * i + a * a), r = (t - o) / o, h = i * 0.5 * r, c = a * 0.5 * r;
  ...
  calculateTotalLength() {
    const t = this.points[0], e = this.points[this.points.length - 1], s = e.x - t.x, n = e.y - t.y;
    return Math.sqrt(s * s + n * n) * this.config.slack;
  }
  render() {
    this.segmentElements.forEach((t, e) => {
      const s = this.points[e], n = this.points[e + 1], i = n.x - s.x, a = n.y - s.y, o = Math.sqrt(i * i + a * a), r = Math.atan2(a, i);
      t.style.width = `${o}px`, t.style.left = `${s.x}px`, t.style.top = `${s.y}px`, t.style.transform = `rotate(${r}rad)`;
    }), this.config.draggable && (this.updateDragHandlePosition(this.startHandle, this.points[0]), this.updateDragHandlePosition(this.endHandle, this.points[this.points.length - 1]));
  }
  updateDragHandlePosition(t, e) {
    t.style.left = `${e.x - this.config.dragHandleSize / 2}px`, t.style.top = `${e.y - this.config.dragHandleSize / 2}px`;
  }
}
class m {
  /**
  ...
  createRootElement() {
    const t = document.createElement("div");
    return t.classList.add("patchbay-container"), t.style.position = "fixed", t.style.top = "0", t.style.bottom = "0", t.style.left = "0", t.style.right = "0", t.style.width = "100%", t.style.height = "100%", t.style.zIndex = this.config.zIndex || 9999, t.style.pointerEvents = "none", t;
  }
  /**
